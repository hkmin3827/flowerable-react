...
홈페이지에서 챗봇 카드를 클릭하면 드로어가 열리며 꽃 추천 대화를 시작합니다.
프론트엔드는 Spring 서버를 통해 Python AI 서버와 통신합니다.

```

홈페이지 챗봇 카드 클릭 → AIChatbotDrawer 오픈 (Zustand)
└─ 사용자 메시지 입력
└─ POST /api/v1/ai/recommend → Spring
└─ Spring → Python AI 서버 (localhost:8000, EC2 내부 통신)
└─ RecommendResponse 반환
├─ recommendation / message → 채팅 말풍선 출력
└─ shops → 추천 꽃집 카드 출력 (클릭 시 꽃집 상세 이동)

```

Python AI 서버는 외부에 직접 노출되지 않으며, EC2 내에서 Spring이 중간 통로 역할을 합니다.
AI 응답 대기 타임아웃은 40초로 설정되어 있습니다.
...
유저와 꽃집이 주문 전·후로 직접 채팅할 수 있습니다.

```
유저 → 꽃집 상세 페이지에서 문의 시작
  └─ 채팅방 생성 또는 기존 채팅방 입장
       └─ 실시간 메시지 송수신
            └─ 꽃집 측 /shop/orders 또는 /chats 에서 수신 확인 및 답변
```
